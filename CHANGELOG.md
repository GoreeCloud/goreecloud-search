...
- VPS Docker Development candidate with a separate container-network listener, bounded Host validation for `search.goreecloud.com`, protected runtime credential-file input, non-root/read-only compatible image, health check, hardened Compose service example, preserved `searxng-core:8080` Caddy compatibility, and no host-port publication.
- Private VPS Search web UI with server-rendered results, POST-based ordinary form search, OpenSearch-compatible GET integration, configured HTTPS service-origin advertisement, escaped provider/query content, provenance/disclosure/explanation presentation, local-only CSS, no client-side JavaScript, and Glaze UI 1.5.1-targeted accessibility/resilience fallbacks.
