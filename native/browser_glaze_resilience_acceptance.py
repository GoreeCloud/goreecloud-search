        ...
        try:
            for path, marker, targets, name in PAGES:
                ...
                if root.get_attribute("data-glaze-version") != "1.3":
                    raise AssertionError(f"{context}: GLAZE UI V1.3 contract was lost")
                assert_no_horizontal_overflow(driver, context)
                assert_controls(driver, targets, context)
                capture(driver, f"glaze-v1-3-rtl-{viewport.name}-{name}")
        finally:
            driver.quit()
...
def assert_two_x_scale_reflow() -> None:
    ...
    try:
        ...
        for path, marker, targets, name in PAGES:
            ...
            assert_no_horizontal_overflow(driver, context)
            assert_controls(driver, targets, context)
            capture(driver, f"glaze-v1-3-2x-scale-{name}")
    finally:
...
def main() -> int:
    ...
    print("native GLAZE UI V1.3 resilience browser acceptance passed")
