...
- Central result normalization validates the actual provider result/open URL even when a separate canonical identity is supplied, rejecting credential-bearing, raw control-bearing, and bidirectional-formatting-bearing HTTP(S) targets before they can survive into normalized user-facing results.
