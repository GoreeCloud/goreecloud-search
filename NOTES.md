...
- The first run does not authorize deployment or establish private publication acceptance. Its DNS lookup occurred on the VPS, not on the owner laptop, so public resolver answers there do not independently prove a laptop private-DNS failure. The corrected Caddy/private-address readback, a separate owner-laptop NetBird + AdGuard private-DNS check, recoverable backup/archive, rollback readiness, live-provider validation, and native target-runtime acceptance remain open.
