...
- The first live preflight run on September 18, 2026 verified the current Compose hash/services/images, active `searxng-core` image identity, runtime hardening gaps, proxy/legacy-Valkey network state, absence of the planned Brave runtime secret file, DNS answers, and a TLS-verified HTTP 404 from the Search homepage. That run also exposed a collector default mismatch: the current production Caddy tree is `/srv/docker/caddy`, while `/srv/docker/stacks/caddy` is only the standardized future target. The collector default is corrected on this branch and Caddy-route evidence must be re-run before any target-host change.
...
- The first run does not authorize deployment or establish private publication acceptance. The corrected Caddy readback, approved private-DNS client path, recoverable backup/archive, rollback readiness, live-provider validation, and native target-runtime acceptance remain open.
