...
- A child release-preparation branch adds a manual GHCR publication workflow. The workflow is designed to fail closed unless invoked from authoritative `main` at an exact requested SHA/version, so its presence on a stacked Draft branch does not authorize or perform publication.
