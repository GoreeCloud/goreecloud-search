...
- `deploy/vps/read-only-preflight.sh` is a pre-cutover evidence collector only. It performs no Docker/Compose/Caddy/file mutation, does not read provider secret values or container environment, does not emit raw Compose/Caddy contents or logs, performs no provider query, and probes only the Search HTTPS homepage.
- The first live preflight run on September 18, 2026 verified the current Compose hash/services/images, active `searxng-core` image identity, runtime hardening gaps, proxy/legacy-Valkey network state, absence of the planned Brave runtime secret file, VPS-local DNS answers, and a TLS-verified HTTP 404 from the Search homepage. That run also exposed two collector limitations: the current production Caddy tree is `/srv/docker/caddy`, while `/srv/docker/stacks/caddy` is only the standardized future target; and `curl --fail` stopped the collector on the 404 before its final boundary summary. The collector is corrected on this branch to use the production Caddy root, retain HTTP status evidence without aborting on non-2xx responses, classify whether the VPS resolver matches the expected private NetBird address, and perform a second TLS-verified homepage probe directly against the expected private address with `curl --resolve`.
- The same live readback supersedes the older documented Search image as current runtime identity: the running container now references `ghcr.io/goreecloud/goreecloud-search@sha256:960928fcc840dc185f50e87f6276dbafb0b0bd07a6a2197eba9adfe83e03b10a`, with OCI version `2026.8.23-2e04d65` and revision `2e04d650aac7a457c594f64ba704218c168c71a8`. This current live image remains rollback evidence only; it is not source evidence for the rewritten native candidate.
- The corrected live preflight was then run on September 18, 2026 from exact source revision `37f5f278a211a0086229a2376abc70af0a1db962`. It reconfirmed the same Compose hash and Search image/runtime state, found the active production `/srv/docker/caddy/Caddyfile`, and verified that the active file contains both `search.goreecloud.com` and `searxng-core:8080` references. The recursive filename-only match counts were 19 for each string because historical Caddy backup files also retain those references; the evidence establishes current-file string presence, not full semantic validation of every backup.
- The corrected VPS-local system-resolver probe still returned public addresses and HTTP 404 with TLS verification success. A separate TLS-verified `curl --resolve` probe forced `search.goreecloud.com` to the approved NetBird address `100.71.27.119` and returned HTTP 200, establishing that the current private Caddy HTTPS path is reachable from the VPS at the intended private address.
- The corrected run completed its final boundary section and again reported no raw Compose/Caddy content, container environment, container logs, files written, Docker state changes, or Caddy state changes.
- These server-side results do not authorize deployment and do not establish the owner laptop client path. The owner laptop must still be checked through its actual NetBird + AdGuard private-DNS configuration. Recoverable pre-cutover backup/archive evidence, rollback verification, live-provider validation, native artifact publication/deployment, and post-cutover acceptance remain open.
