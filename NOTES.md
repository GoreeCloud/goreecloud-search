...
- The private VPS web surface targets Glaze UI Stable 1.5.1. Source-level semantics/accessibility/resilience controls exist, but no Search-specific rendered, assistive-technology, human, device/browser, or target-runtime acceptance has been recorded. Glaze conformance and production eligibility therefore remain blocked.
- Ordinary human Search form submissions use POST to reduce query exposure in page URLs. OpenSearch/browser integration remains GET-based by protocol and must be evaluated with actual Browser/Caddy history/log behavior before acceptance.
