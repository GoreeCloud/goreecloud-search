...
Result normalization treats provider result/open URLs as independently untrusted from any separately supplied canonical identity. A provider cannot use a safe canonical alias to carry a credential-bearing target or one containing raw whitespace, control/format characters, bidirectional formatting, backslashes, invalid DNS labels, IPv6 zone identifiers, ambiguous numeric host syntax, or explicit port 0 into normalized results. Valid internationalized DNS hosts are canonicalized to ASCII A-label form, canonical four-octet decimal IPv4 is preserved, and valid IPv6 authorities are reconstructed with brackets preserved so canonicalization does not turn a valid parsed authority into an invalid or ambiguous URL. Provider title/snippet display text is separately sanitized and bounded before ranking/presentation so untrusted providers cannot inject terminal-style controls, bidi reordering, or unbounded visible text into normalized results.
