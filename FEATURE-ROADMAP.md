...
Accepted `master` remains the previously merged Development line. Active PR #171 is a newer Development candidate and is not merged, production accepted, or Stable merely because its validation passes.

The latest verified PR #171 implementation checkpoint is `79eb7bdbf168361c6e36e73df953f82500d8cbf0`. All 14 current workflow families passed on that exact revision: Platform Contract #47, Search Native Foundation #295, foundation #1789, runtime smoke #1625, API v1 service contract #150, platform integrations #1125, upstream container boundary #1001, container build #1621, browser acceptance #1714, native development artifact #197, native Development container #168, native results browser acceptance #234, Integration #677, and Documentation #675.
...
- includes a bounded Privacy Shield v1 HTTP verification client with injected authenticated-transport seam, loopback-only cleartext allowance, HTTPS for non-loopback, no redirects, strict JSON, bounded response size, no-store, and timeout controls;
...
The major runtime blocker is still an approved GoreeCloud Identity direct-service authentication path for Search → Privacy Shield plus authenticated Browser/Index requester identity for Browser/Index → Search. Search must not switch to required Privacy Shield enforcement until those identities and the real authority transport are accepted and tested. Query/history synchronization also remains disabled unless explicit user controls and privacy/retention contracts authorize it.
...
| --- | --- | --- | --- |
| FR-001 | Reconcile and maintain every current planned or recommended GoreeCloud Search feature from the authoritative project record and verified repository evidence in this roadmap. | High | Ongoing control; synchronized to the verified September 14 PR #171 checkpoint. |
...
| FR-018 | Define and validate bounded first-party GoreeCloud Index and Browser consumer contracts over the native Search API, including result/request limits, capability evidence, fail-closed preflight, degraded-state preservation, safe executable destinations, and data minimization. | High | Merged foundation plus active PR #171 hardening. Search now publishes the intended production POST/JSON, Privacy Shield reference, and authenticated-requester contract, but live consumer transport and production acceptance remain blocked. |
...
| FR-020 | Complete durable operation-scoped Privacy Shield authorization and authenticated first-party requester/service identity for Browser/Index → Search and Search → Privacy Shield without interpreting authority tokens inside Search. | High | PR #171 implements fail-closed gate/client contracts and independent authority-response validation. Accepted GoreeCloud Identity direct-service authentication, authenticated requester resolution, real runtime wiring, denial/consume/replay acceptance, and `required` enforcement remain blocked. |
...
1. Establish an approved GoreeCloud Identity direct-service authentication contract for Search → Privacy Shield and authenticated requester identity for Browser/Index → Search.
2. Connect the existing Privacy Shield verification client to that accepted authenticated transport, then enable required-mode runtime wiring only after the verifier and requester resolver have exact runtime evidence.
3. Add handler-level denial, consume/single-use, replay, revocation/expiry, and constraint-mismatch acceptance against the real authenticated verification path before advertising `privacy_authorization_enforcement=required`.
