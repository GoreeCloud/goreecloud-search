...
The Development HTTP search endpoints use GET query parameters. The loopback command is local-only; the separate VPS container candidate is intended to receive requests only through the existing private Caddy path and does not publish a host port. A browser, reverse proxy, local client, intermediary debugging tool, or operating-system component outside this server may retain requested URLs according to its own settings. The Search server's `no-store` response and suppressed request-target logging do not control Caddy, browser, or other external histories. Final Caddy access-log handling and Browser/private-search behavior must therefore be verified on the target runtime before production acceptance.
