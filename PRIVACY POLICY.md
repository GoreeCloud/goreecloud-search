...
The Brave adapter does not persist queries, API responses, or search history. The initial web UI is server-rendered, uses no client-side JavaScript, remote fonts, analytics, trackers, or third-party presentation assets, and HTML-escapes query/provider-controlled text before rendering. External-provider operation remains subject to the provider's own service and privacy terms and is not equivalent to GoreeCloud-only search.

The Development JSON API and explicit OpenSearch/browser integration use GET query parameters. The human Search page instead submits ordinary searches as bounded `application/x-www-form-urlencoded` POST requests so the query is not placed in the page URL. The loopback command is local-only; the VPS container candidate is intended to receive requests only through the existing private Caddy path and does not publish a host port. Browser integrations that use the OpenSearch GET template, reverse-proxy behavior, local clients, debugging tools, or operating-system components may still retain requested URLs according to their own settings. The Search server's `no-store` response and suppressed request-target logging do not control Caddy, browser, or other external histories. Final Caddy access-log handling and Browser/private-search behavior must therefore be verified on the target runtime before production acceptance.
